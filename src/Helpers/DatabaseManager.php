    ...
    private function seed(BufferedOutput $outputLog)
    {
        try {
            ...
            if (!empty($other_commands)) {
                foreach ($other_commands as $command => $options) {
                    if (is_array($options)) {
                        // Execute the command with the specified options
                        Artisan::call($command, $options, $outputLog);
                    } else {
                        // Execute the command without options or arguments
                        Artisan::call($options, [], $outputLog);
                    }
                }
            }
        } catch (Exception $e) {
